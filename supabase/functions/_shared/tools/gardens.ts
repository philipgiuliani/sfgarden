...
import { colToLetter, letterToCol } from "../utils/grid.ts";
...
export function registerGardenTools(
  ...
  server.tool(
    ...
    async () => {
      ...
      if (!gardens || gardens.length === 0) {
        return {
          content: [{ type: "text" as const, text: "No gardens found. Use sfg_create_garden to create one." }],
        };
      }
  ...
  server.tool(
    ...
    async ({ name, cols, rows, notes }) => {
      ...
      return {
        content: [{ type: "text" as const, text: `Garden "${name}" created (${data.id}, ${cols}x${rows}).` }],
      };
    },
  );
}
