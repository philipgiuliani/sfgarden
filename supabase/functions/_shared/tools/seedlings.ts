...
export function registerSeedlingTools(
  ...
  server.tool(
    ...
    async ({ plant_name, variety, count, sown_at, notes }) => {
      ...
      if (userErr || !user) {
        return { content: [{ type: "text" as const, text: "Error: Could not resolve current user." }], isError: true };
      }
  ...
  server.tool(
    ...
    async ({ seedling_id, phase, planting_id, date }) => {
      ...
      if (fetchErr || !seedling) {
        return {
          content: [{ type: "text" as const, text: `Error: Seedling ${seedling_id} not found.` }],
          isError: true,
        };
      }
