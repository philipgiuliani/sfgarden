...
export function registerPlantingTools(
  ...
  server.tool(
    ...
    async ({ garden_id, squares, plant_name, variety, count, planted_at, notes }) => {
      ...
      if (gardenErr || !garden) {
        return { content: [{ type: "text" as const, text: `Error: Garden ${garden_id} not found.` }], isError: true };
      }
      ...
      for (const label of labels) {
        ...
        if (error) {
          return { content: [{ type: "text" as const, text: `Error creating planting for ${label}: ${error.message}` }], isError: true };
        }
  ...
  server.tool(
    ...
    async ({ planting_id, status }) => {
      ...
      if (error || !data) {
        return {
          content: [{ type: "text" as const, text: `Error: ${error?.message ?? "Planting not found."}` }],
          isError: true,
        };
      }
